...
struct FSynthesizedTrace {
  ...
  explicit FSynthesizedTrace(std::string source_name)
      : trace(std::move(source_name)) {}
};
...
class FTraceSynthesizer {
 public:
  [[nodiscard]] static FSynthesizedTrace Generate(
      const FTraceSynthesizerConfig& InConfig);
};
