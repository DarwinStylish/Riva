...
JsonTraceAdapter::JsonTraceAdapter(JsonTraceLoaderOptions options)
    : options_(options) {}

std::string JsonTraceAdapter::name() const {
  return "json";
}
...
TraceAdapterResult JsonTraceAdapter::Load(const std::string& path) const {
  const auto result = LoadNormalizedTraceFromJsonFile(path, options_);
