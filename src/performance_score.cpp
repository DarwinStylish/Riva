...
FSubsystemScore ScoreSubsystem(
  ...
  if (p95_ms <= 0.001) {
    // No data for this subsystem — full marks, no deductions
...
FPerformanceScore ComputePerformanceScore(
    const FTraceStatistics& stats,
  ...
  result.subsystems.push_back(ScoreSubsystem(
      "Game Thread", stats.game_thread_p95_ms, config.target_frame_ms, config.p95_penalty_factor));
  result.subsystems.push_back(ScoreSubsystem(
      "Render Thread", stats.render_thread_p95_ms, config.target_frame_ms, config.p95_penalty_factor));
  result.subsystems.push_back(ScoreSubsystem(
      "GPU", stats.gpu_p95_ms, config.target_frame_ms, config.p95_penalty_factor));
  ...
  if (stats.ai_p95_ms > 0.001) {
    result.subsystems.push_back(ScoreSubsystem(
        "AI", stats.ai_p95_ms, config.target_frame_ms * 0.2, config.p95_penalty_factor));
  }
  ...
  double worst_subsystem = 100.0;
  for (const auto& sub : result.subsystems) {
    if (sub.score < worst_subsystem) {
