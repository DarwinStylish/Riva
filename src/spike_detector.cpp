...
RollingMedianSpikeDetector::RollingMedianSpikeDetector(SpikeDetectionConfig config)
    : config_(config) {}

const SpikeDetectionConfig& RollingMedianSpikeDetector::config() const noexcept {
  return config_;
}
...
std::vector<Spike> RollingMedianSpikeDetector::Detect(const NormalizedTrace& trace) const {
  ...
  for (std::size_t i = 0; i < frames.size(); ++i) {
    ...
    if (frame_ms >= config_.absolute_threshold_ms &&
        ratio >= config_.ratio_threshold &&
