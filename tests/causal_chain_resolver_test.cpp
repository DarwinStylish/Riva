...
Finding MakeFinding(std::string id, std::size_t frame_index, double confidence) {
  Finding r;
  r.id = id;
...
void TestResolvesCausalChain() {
  ...
  AssertTrue(output[0].confidence > output[1].confidence, "Root cause confidence should be higher than symptom");
}

void TestIgnoresDifferentFrames() {
  std::vector<Finding> input = {
    MakeFinding("STUT_GC", 1, 0.6),
    MakeFinding("STUT_CPU_GT", 2, 0.8)
  };
