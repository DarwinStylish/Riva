...
AnalysisResult AnalysisEngine::Analyze(const NormalizedTrace& trace) const {
  ...
  for (const auto& signature : signatures_) {
    ...
    raw_findings.insert(raw_findings.end(), std::make_move_iterator(signature_findings.begin()),
  ...
  if (config_.budget.has_value()) {
    ...
    for (const auto& frame : trace.frames()) {
      if (budget.game_thread_ms_max.has_value() &&
          frame.game_thread_ms > *budget.game_thread_ms_max) {
        game_thread_breached = true;
      }
      if (budget.render_thread_ms_max.has_value() &&
          frame.render_thread_ms > *budget.render_thread_ms_max) {
  ...
  // Use the same configured hitch boundary for statistics and scoring so the
  // reported hitch percentage and its score penalty cannot drift apart.
  result.statistics = ComputeTraceStatistics(trace, config_.performance_score.hitch_threshold_ms);
  result.score = ComputePerformanceScore(result.statistics, config_.performance_score);
