...
std::string RoleToString(FindingRole role) {
  switch (role) {
    case FindingRole::kPrimary:
      return "Primary";
    case FindingRole::kSecondary:
      return "Secondary";
  }
  return "Secondary";
}
...
std::string EvidenceClassificationToString(EEvidenceClassification classification) {
  switch (classification) {
    case EEvidenceClassification::kObserved:
      return "OBSERVED";
    case EEvidenceClassification::kDerived:
      return "DERIVED";
    case EEvidenceClassification::kCorrelated:
      return "CORRELATED";
    case EEvidenceClassification::kInferred:
      return "INFERRED";
    case EEvidenceClassification::kSuspected:
      return "SUSPECTED";
    case EEvidenceClassification::kRecommended:
      return "RECOMMENDED";
  }
  return "OBSERVED";
}
...
Status FReportEngine::GenerateReport(EReportFormat InFormat, const AnalysisResult& InResult,
...
Status FReportEngine::GenerateMarkdownReport(const AnalysisResult& InResult,
  ...
  if (InResult.statistics.total_frames > 0) {
    ...
    ss << "- **Overall Score**: " << std::fixed << std::setprecision(0) << InResult.score.overall
       << " / 100 (" << InResult.score.overall_grade << ")\n";
    ...
    ss << "- **Hitch Rate**: " << std::setprecision(1) << InResult.statistics.hitch_percentage
       << "% (" << InResult.statistics.hitch_count << " of " << InResult.statistics.total_frames
       << " frames)\n\n";
    ...
    if (!InResult.score.subsystems.empty()) {
      ...
      for (const auto& sub : InResult.score.subsystems) {
        ss << "| " << sub.name << " | ";
        if (!sub.has_data) {
          ss << "N/A | N/A | Telemetry unavailable";
        } else if (sub.deductions.empty()) {
          ss << std::setprecision(0) << sub.score << " | " << sub.grade << " | Within target";
        } else {
          ss << std::setprecision(0) << sub.score << " | " << sub.grade << " | "
             << sub.deductions[0];
        }
        ss << " |\n";
      }
  ...
  if (InResult.findings.empty()) {
    ...
    for (const auto& resolved : InResult.findings) {
      ...
      ss << "### " << index++ << ". [" << SeverityToString(f.severity) << "] " << f.title
         << " (ID: " << f.id << ")\n";
      ss << "- **Role**: " << RoleToString(resolved.role) << "\n";
      ss << "- **Confidence**: " << FormatConfidence(f.confidence) << "\n";
      ss << "- **Frame Index**: " << f.frame_index << "\n";
      ss << "- **Time Window**: " << f.time_window_start_us << " us - " << f.time_window_end_us
         << " us\n";
      ...
      if (InOptions.include_evidence_details && !f.evidence.empty()) {
        ...
        for (const auto& ev : f.evidence) {
          ss << "- `" << ev.label << "` [" << EvidenceClassificationToString(ev.classification)
             << "]: " << ev.value << "\n";
        }
        ss << "\n";
      }
...
Status FReportEngine::GenerateJsonReport(const AnalysisResult& InResult,
                                         const FReportOptions& InOptions, std::string& OutJson) {
  std::ostringstream ss;
  ss << "{\n";
  if (InOptions.include_metadata) {
      ...
      if (InOptions.include_evidence_details) {
        ...
        if (f.evidence.empty()) {
          ...
          for (std::size_t j = 0; j < f.evidence.size(); ++j) {
            ...
            ss << "          \"classification\": \""
               << EvidenceClassificationToString(ev.classification) << "\"\n";
  ...
  if (close_pos != std::string::npos && InResult.statistics.total_frames > 0) {
    ...
    extra << "    \"hitch_percentage\": " << std::setprecision(1)
          << InResult.statistics.hitch_percentage << "\n";
    extra << "  },\n";
    extra << "  \"performance_score\": {\n";
    extra << "    \"has_data\": " << (InResult.score.has_data ? "true" : "false") << ",\n";
    ...
    for (std::size_t i = 0; i < InResult.score.subsystems.size(); ++i) {
      ...
      extra << "        \"has_data\": " << (sub.has_data ? "true" : "false") << ",\n";
...
Status FReportEngine::GenerateComparisonReport(const ComparisonResult& InResult,
  ...
  FormatFindingSection("Improvements", InResult.improvements,
  ...
  if (!InResult.statistics.metric_deltas.empty()) {
    ...
    for (const auto& md : InResult.statistics.metric_deltas) {
      ...
      if (!md.bPercentageDefined) {
        pct_ss << "n/a (zero baseline)";
      } else {
      ...
      if (md.bRegressed) {
        if (!md.bPercentageDefined || md.delta_percent > 25.0) {
          indicator = " 🔴";
        } else {
          indicator = " ⚠️";
        }
      ...
      ss << "| " << md.metric_name << " | " << baseline_ss.str() << unit << " | " << new_ss.str()
         << unit << " | " << delta_ss.str() << unit << " | " << pct_ss.str() << indicator << " |\n";
    }
    ss << "\n";
  }
