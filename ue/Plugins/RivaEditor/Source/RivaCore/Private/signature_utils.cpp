...
SpikeContext BuildSpikeContext(const NormalizedTrace& trace, const Spike& spike,
                               std::uint64_t padding_us) {
  SpikeContext context;

  const auto frame =
      std::lower_bound(trace.frames().begin(), trace.frames().end(), spike.frame_index,
                       [](const Frame& candidate, std::size_t frame_index) {
                         return candidate.index < frame_index;
                       });
  if (frame == trace.frames().end() || frame->index != spike.frame_index) {
    return context;
  }
  ...
  context.window_end_us = padding_us > std::numeric_limits<std::uint64_t>::max() - frame_end
                              ? std::numeric_limits<std::uint64_t>::max()
                              : frame_end + padding_us;

  return context;
}

std::vector<const TraceEvent*> EventsInWindow(const Frame& frame, std::uint64_t window_start_us,
...
bool EventMatchesAny(const TraceEvent& event, const std::vector<std::string>& needles) {
  const std::string combined = event.name + " " + event.category + " " + event.thread_name;
