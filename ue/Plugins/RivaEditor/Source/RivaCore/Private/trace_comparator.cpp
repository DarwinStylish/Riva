...
FMetricDelta MakeDelta(const std::string& name, double baseline, double new_val,
  ...
  if (std::abs(baseline) > 0.001) {
    delta.delta_percent = (delta.delta / std::abs(baseline)) * 100.0;
    delta.bRegressed = delta.delta_percent > threshold_percent;
  } else {
    // Percentage change from zero is undefined. Preserve that fact in the
    // model and use the absolute increase to avoid hiding regressions.
    delta.bPercentageDefined = false;
    delta.delta_percent = 0.0;
    delta.bRegressed = delta.delta > 0.001;
  }
  return delta;
}
...
ComparisonResult DefaultTraceComparator::Compare(const NormalizedTrace& baseline_trace,
  ...
  result.statistics.metric_deltas.push_back(
      MakeDelta("Hitch %", b.hitch_percentage, n.hitch_percentage, 1.0));
  result.statistics.metric_deltas.push_back(
      MakeDelta("Game Thread P95", b.game_thread_p95_ms, n.game_thread_p95_ms));
  result.statistics.metric_deltas.push_back(
      MakeDelta("Render Thread P95", b.render_thread_p95_ms, n.render_thread_p95_ms));
  result.statistics.metric_deltas.push_back(
      MakeDelta("RHI Thread P95", b.rhi_thread_p95_ms, n.rhi_thread_p95_ms));
  ...
  if (b.physics_p95_ms > 0.001 || n.physics_p95_ms > 0.001) {
    result.statistics.metric_deltas.push_back(
        MakeDelta("Physics P95", b.physics_p95_ms, n.physics_p95_ms));
  }
  ...
  if (b.network_p95_ms > 0.001 || n.network_p95_ms > 0.001) {
    result.statistics.metric_deltas.push_back(
        MakeDelta("Network P95", b.network_p95_ms, n.network_p95_ms));
  }
  if (b.loading_p95_ms > 0.001 || n.loading_p95_ms > 0.001) {
    result.statistics.metric_deltas.push_back(
        MakeDelta("Loading P95", b.loading_p95_ms, n.loading_p95_ms));
  }
